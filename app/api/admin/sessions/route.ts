...
export async function GET() {
  ...
  const [{ data: sessions, error: sessionsError }, { data: modules }] = await Promise.all([
    supabase.from("sessions").select("id, title, order_index, module_id, main_video_provider, main_video_bunny_id, main_video_vdocipher_id").order("order_index"),
  ...
  const result = sessions
    .map((s) => ({
      ...
      main_video_bunny_id: s.main_video_bunny_id,
