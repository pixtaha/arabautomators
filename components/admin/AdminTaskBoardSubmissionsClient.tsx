...
// TaskBoardClient.tsx, counted per WORD rather than per character -- a
// character-count comparison breaks down on text that's Arabic prose
// heavily larded with English jargon or a raw URL (a single URL can rack
// up far more Latin *characters* than the surrounding Arabic sentence has,
// even though a human reads the whole thing as Arabic). Classifying by
// whole words (a word counts as Arabic if it contains any Arabic
// character at all) tracks how the text actually reads much more
// reliably. Ties resolve to Arabic, not Latin -- this platform's content
// is Arabic-first with English technical terms mixed in. Not imported
// from TaskBoardClient.tsx because it's a local, non-exported helper
// there too -- matches this file's existing convention of duplicating
// small view-layer helpers (formatDate/formatSize above,
...
export function AdminTaskBoardSubmissionsClient() {
